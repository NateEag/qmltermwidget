...
QKeyEvent * Vt102Emulation::remapKeyModifiersForMac(QKeyEvent *event) {
  ...
  // Restoring the below logic from 5c47d1f49455394 makes mod keys work for
  // basic Option key combos, I *think* by detecting key events that would
  // otherwise be interpreted by macOS as special character entry and
  // converting them into something else. However, whatever exactly it's doing,
  // it fails for Option+Shift key combinations, which are used heavily in
  // Emacs.
  //
  // Surely there's a better answer than overriding every possible key
  // combination involving Option?
  //
  // FIXME Figure out a sane way to bypass macOS's default Option keybindings.
